...
def main():
    ...
    parser.add_argument('-q', '--quiet', action='store_true', help='Do not print logging to stdout')
    ...
    for indir in config['watch']:
        if os.path.isdir(indir):
            ...
            root.info("started watching " + indir + " with out at " + config['watch'][indir])
        else:
...
def convert_file(fname, outdir, config):
    ...
    with magic.Magic(flags=magic.MAGIC_MIME_TYPE) as m:
        ...
        if ret and config['copy_original']:
            logging.info("Copy original file "+ fname + " to destination "+outdir)
